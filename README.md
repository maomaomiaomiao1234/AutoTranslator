...
- **划词翻译**：鼠标选中文字后自动弹出翻译浮窗。优先通过 Accessibility API 获取选区，失败时临时模拟 `⌘C`，并在读取后恢复原剪贴板内容（剪贴板回退可在偏好设置中关闭）。
...
默认选择大模型后端。未配置大模型 API Key 时：macOS 15+ 自动改用 Apple 系统翻译（本机完成，不发送文本到第三方）并通知；更早的系统会在翻译时提示先配置 API Key 或切换后端。应用不会把文本静默发送给未选择的服务。

> Google 后端使用的是非官方公开 Web 接口，可能受到网络环境、访问频率或服务变更影响，不适合依赖稳定 SLA 的生产场景；仅在用户显式选择该后端时才会使用。
...
2. 按提示授予「辅助功能」权限；授权后划词翻译会自动开始工作，无需重启应用。授权前，「翻译输入」「截图翻译」「翻译历史」等菜单功能仍可正常使用，菜单栏也提供「打开辅助功能设置…」入口。
...
| 权限 | 用途 |
| --- | --- |
| 辅助功能 | 监听全局鼠标事件、读取选中文字，并在必要时模拟复制快捷键（可关闭） |
| 屏幕与系统音频录制 | 仅在主动使用「截图翻译」时截取用户框选的屏幕区域，用于本机 OCR |
| 通知 | 显示后端切换、暂停监听和配置保存等状态 |

辅助功能权限是划词监听的前提。未授权时应用不会退出，而是驻留菜单栏等待授权，授权到位后自动启用划词。
...
| 变量 | 说明 | 默认值 |
| --- | --- | --- |
| `TRANSLATOR_BACKEND` | `llm`、`google` 或 `apple` | `llm`；无 Key 时 macOS 15+ 改用 `apple`，否则提示配置 |
| `CLIPBOARD_FALLBACK` | `0`、`false`、`no` 或 `off` 时关闭剪贴板回退取词 | 开启 |
